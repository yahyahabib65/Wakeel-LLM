...
def model_1_response(query):
    return f"Model 1 says: Here's a legal insight for *{query}*."

def model_2_response(query):
    return f"Model 2 responds: Based on legal precedence, *{query}* is addressed like this."
...
with tab1:
    ...
    model_choice = st.radio("Choose Legal Model", ["Model 1", "Model 2"], horizontal=True)
    ...
    with chat_placeholder:
        for chat in st.session_state["chat_history"]:
            if chat["role"] == "user":
                ...
                st.markdown(f"**🤖 Wakeel ({chat['model']}):** {chat['content']}")
                st.caption(chat['time'])
                st.button("👍", key=f"like_{chat['time']}")
                st.button("👎", key=f"dislike_{chat['time']}")
                st.download_button("📥 Download PDF", chat["content"], file_name="legal_response.pdf")
    ...
    if send and user_input:
        ...
        if model_choice == "Model 1":
            ai_response = model_1_response(user_input)
        else:
            ai_response = model_2_response(user_input)
...
with tab2:
    ...
    st.markdown("- Draft a rental agreement between landlord and tenant in Punjab.")
    st.markdown("- Create an employment contract for a software engineer.")
    st.markdown("- Generate a power of attorney for property sale.")
    st.markdown("- Write a legal notice for payment default.")
